...
<%@page import="com.briup.crm.common.bean.CstCustomer"%>
...
<body>
	<%PageInfo<CstCustomer>  pageInfoList = (PageInfo<CstCustomer>) session.getAttribute("pageInfoList"); %>
	...
	<form action="cust_info/select_customer" method="post" id="queryform">
	...
	<table id="data_list_table" class="data_list_table" cellSpacing=1
		...
		<c:forEach	items="${pageInfoList.list }" var="cust" >
		
			<tr align="center">
				<td class="list_data_number">${cust.custId }</td>
				<td class="list_data_text">${cust.custNo }</td>
				<td class="list_data_ltext">${cust.custName }</td>
				<td class="list_data_text">${cust.custRegion }</td>
				<td class="list_data_text">${cust.custManagerName }</td>
				<td class="list_data_text">${cust.custLevelLabel }</td>
				<td class="list_data_op">
				<c:choose>
				<c:when test="${roleId!=4 }">
				<img
					onClick="to('cust_info/customer_edit/${cust.custId }');" title="编辑"
				...
				<c:otherwise>
					...
					<img
					onClick="to('cust_info/linkman/${cust.custId }');" title="联系人"
					src="images/bt_linkman.gif" class="op_button" /> 
					<img
					onClick="to('cust_info/activities/${cust.custNo}');" title="交往记录"
					src="images/bt_orders.gif"> 
				<c:choose>
				<c:when test="${roleId!=4 }">
					<!-- 经理只能删除属于自己的客户 --> 
					<a
					href="javascript:if(confirm('确认删除？')){window.location.href='cust_info/delete_customer/${cust.custId }'}">	
		...
		<tr>
			...
			<a href="cust_info/list/1">首页</a>
			<c:choose>
				<c:when test="${sessionScope.pageInfoList.hasPreviousPage }">
					<a href="cust_info/list/${pageInfoList.prePage }">上一页</a>
				</c:when>
				<c:otherwise>
			...
			<c:choose>
				<c:when test="${sessionScope.pageInfoList.hasNextPage }">
					<a href="cust_info/list/${pageInfoList.nextPage }">下一页</a>
				</c:when>
				<c:otherwise>
					<font color="#ABA8AB">下一页</font>
				</c:otherwise>
			</c:choose>
				 <a href="cust_info/list/${sessionScope.pageInfoList.pages }">尾页</a>
			</th>
		</tr>
	</table>
</body>
