...
<body>
   <br>
	...
	<form action="cust_service/select_service" method="post" name="frm"  id="queryform">
	...
	<table id="data_list_table" class="data_list_table" cellSpacing=1
			...
			<c:forEach	items="${pageInfoList.list }" var="ser" >
				<tr align="center">
					...
					<td class="list_data_text">
								<form action="cust_service/dispatch_update" method="post" name="frm" id="fenpeiform">
									<input type="hidden" value="${ser.svrId }" name="svrId">
									<select name="svrDueTo" id="cs.svr_due_to">
										<option value="${ser.svrDueTo }">${ser.svrDueTo }</option>
										<option value="lili">lili</option>
										<option value="jinli">jinli</option>
					...
					<td class="list_data_op"><a
						href="cust_service/delete_service/${ser.svrId }"><img
						title="删除"
		...
		<tr>
			...
			<a href="cust_service/dispatch/1">首页</a>
			<c:choose>
				<c:when test="${sessionScope.pageInfoList.hasPreviousPage }">
					<a href="cust_service/dispatch/${pageInfoList.prePage }">上一页</a>
				</c:when>
				<c:otherwise>
			...
			<c:choose>
				<c:when test="${sessionScope.pageInfoList.hasNextPage }">
					<a href="cust_service/dispatch/${pageInfoList.nextPage }">下一页</a>
				</c:when>
				<c:otherwise>
					<font color="#ABA8AB">下一页</font>
				</c:otherwise>
			</c:choose>
				 <a href="cust_service/dispatch/${sessionScope.pageInfoList.pages }">尾页</a>
			</th>
		</tr>
	</table>
</body>
